...
const spermogramme = require("../models/spermogramme");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await spermogramme.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await spermogramme.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new spermogramme(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
