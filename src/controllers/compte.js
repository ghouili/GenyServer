...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await Compte.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new Compte(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.raison_sociale = raison_sociale;
    existData.adresse = adresse;
    existData.telephone = telephone;
    existData.fax = fax;
    existData.email = email;
    existData.langue = langue;
    existData.responsable = responsable;
    existData.categories_consultations = categories_consultations;
    existData.distribution = distribution;
