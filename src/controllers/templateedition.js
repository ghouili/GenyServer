...
const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await templateedition.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await templateedition.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new templateedition(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.categorie_consultation = categorie_consultation;
    existData.motif_consultation = motif_consultation;
    existData.libelle = libelle;
    existData.contenu = contenu;
