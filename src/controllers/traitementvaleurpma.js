 ...
 const  traitementvaleurpma = require("../models/traitementvaleurpma");

 const GetAllData = async (req, res) => {
     let AllData;
     try {
         AllData = await  traitementvaleurpma.find();
     } catch (error) {
 ...
 const FindDataById = async (req, res) => {
     ...
     let existData;
     try {
         existData = await  traitementvaleurpma.findById(id);
     } catch (error) {
         return res.status(500).json({
             success: false,
             message: "somethin went wrong while extracting data",
 ...
 const AddData = async (req, res) => {
     ...
     const newData = new  traitementvaleurpma(req.body);
     try {
         await newData.save();
     } catch (error) {
         return res.status(500).json({
 ...
 const UpdateData = async (req, res) => {
     ...
     existData.traitement = traitement ;
     existData.suivi = suivi ;
     existData.rang = rang ;
