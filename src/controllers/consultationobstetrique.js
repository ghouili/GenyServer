...
const concultationobs = require("../models/consultationobstetrique");

const GetAllData = async (req, res) => {
  let AllData;
  try {
    AllData = await concultationobs.find();
  } catch (error) {
...
const FindDataById = async (req, res) => {
  ...
  let existData;
  try {
    existData = await concultationobs.findById(id);
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
  ...
  const newData = new concultationobs(req.body);
  try {
    await newData.save();
  } catch (error) {
    return res.status(500).json({
