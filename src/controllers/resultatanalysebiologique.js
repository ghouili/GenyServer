...
const resanalysebio = require("../models/resultatanalysebiologique");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await resanalysebio.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await resanalysebio.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new resanalysebio(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.analyse = analyse;
    existData.prescription = prescription;
    existData.date = date;
    existData.valeur = valeur;
    existData.observation = observation;
    existData.valeur_anormale = valeur_anormale;
