...
const AnalyseBiologique = require("../models/analysebiologique");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await AnalyseBiologique.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await AnalyseBiologique.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new AnalyseBiologique(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.libelle = libelle;
    existData.code = code;
    existData.type = type;
    existData.unite = unite;
    existData.modele_resultat = modele_resultat;
    existData.ordre = ordre;
