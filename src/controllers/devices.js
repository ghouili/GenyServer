...
const FindDataById = async (req, res) => {
  ...
  let existData;
  try {
    existData = await devices.findById(id);
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
  ...
  const newData = new devices(req.body);
  try {
    await newData.save();
  } catch (error) {
    return res.status(500).json({
...
const UpdateData = async (req, res) => {
  ...
  existData.marque =marque;
  existData.modele =modele;
  existData.mise_circulation =mise_circulation;
  existData.informations =informations;
  existData.reference =reference;
  existData.ae_title =ae_title;
  existData.ip =ip;
  existData.port =port;
