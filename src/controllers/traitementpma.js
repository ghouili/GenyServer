...
const traitementpma = require("../models/traitementpma");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await traitementpma.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await traitementpma.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new traitementpma(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
