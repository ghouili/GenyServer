...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await profile.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const UpdateData = async (req, res) => {
    ...
    existData.titre = titre;
    existData.code_conventionnel = code_conventionnel;
    existData.date_naissance = date_naissance;
    existData.telephone_principal = telephone_principal;
    existData.telephone_secondaire = telephone_secondaire;
    existData.signature = signature;
    existData.ajouter_signature_edition = ajouter_signature_edition;
    existData.code_securite_sociale = code_securite_sociale;
    existData.enligne = enligne;
    existData.default_device = default_device;
