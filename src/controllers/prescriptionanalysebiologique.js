...
const prescriptionanalysebiologique = require("../models/prescriptionanalysebiologique");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await prescriptionanalysebiologique.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await prescriptionanalysebiologique.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new prescriptionanalysebiologique(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.analyses = analyses;
    existData.date_prescription = date_prescription;
    existData.date_resultat = date_resultat;
