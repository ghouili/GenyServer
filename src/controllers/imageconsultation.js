...
const imageconsultation = require("../models/imageconsultation");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await imageconsultation.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await imageconsultation.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new imageconsultation(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.date = date;
    existData.impression = impression;
