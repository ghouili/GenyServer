...
const BilanEndocrinienPMA = require("../models/bilanendocrinienpma");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await BilanEndocrinienPMA.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await BilanEndocrinienPMA.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new BilanEndocrinienPMA(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
