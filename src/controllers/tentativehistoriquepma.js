...
const tentativehistoriquepma = require("../models/tentativehistoriquepma");

const GetAllData = async (req, res) => {
    let AllData;
    try {
        AllData = await tentativehistoriquepma.find();
    } catch (error) {
...
const FindDataById = async (req, res) => {
    ...
    let existData;
    try {
        existData = await tentativehistoriquepma.findById(id);
    } catch (error) {
        return res.status(500).json({
            success: false,
            message: "somethin went wrong while extracting data",
...
const AddData = async (req, res) => {
    ...
    const newData = new tentativehistoriquepma(req.body);
    try {
        await newData.save();
    } catch (error) {
        return res.status(500).json({
...
const UpdateData = async (req, res) => {
    ...
    existData.interrogatoire = interrogatoire;
    existData.date = date;
    existData.acte = acte;
    existData.resultat = resultat;
    existData.remarques = remarques;
